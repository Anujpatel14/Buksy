...
const { buildExecutionPlan } = require("./autonomy");
const {
  planChatAction,
  generateBuddyReply
} = require("./ollama");
...
function looksLikeTwinSimulation(message) {
  return /\b(will i actually|what would i likely|simulate me|simulate this plan|predict me)\b/i.test(message);
}

function looksLikeAutopilotPreview(message) {
  return /\b(autopilot|rebalance my day|take over planning|plan everything for me)\b/i.test(message);
}

function looksLikeFutureTimeline(message) {
  return /\b(future timeline|if i continue this pace|6 months|aggressive mode|balanced mode|lazy mode)\b/i.test(message);
}

function looksLikeStrategyRequest(message) {
  return /\b(where should i invest time|highest roi|ceo mode|strategic focus)\b/i.test(message);
}
...
function looksLikeExecutionRequest(message) {
  return /^(please\s+)?(send|schedule|book|sync|execute|run)\b/i.test(message) ||
    (/\bfollow[- ]?up email\b/i.test(message) && /\bsend\b/i.test(message));
}
...
async function executeAutonomousRequest(state, style, sourceMessage) {
  await recordChatPattern(sourceMessage, "plan");
  const plan = buildExecutionPlan({ message: sourceMessage }, state);
  ...
  return replyAndStore(
    ...
    formatReply(
      nextStyle,
      `I queued "${execution.title}" in the action center. Review it once, then approve it and Buksy will run it for you.`,
      `Queued "${execution.title}" for approval.`
    ),
    { execution }
  );
}
...
async function executeGoalCreation(state, style, sourceMessage, input = {}) {
  ...
  const goalPlan = buildAutonomousGoalPlan(state, {
    title,
    targetDays,
    objective: input.objective || `Autogenerated from chat: ${sourceMessage}`
  }, buildRuntimeContext(state));
  const result = await addGoal(goalPlan.blueprint);
  await addArtifact({
  ...
  return replyAndStore(
    ...
    formatReply(
      style,
      `Goal created: "${result.goal.title}". I mapped ${result.tasks.length} starter tasks, estimated about ${goalPlan.dailyHoursNeeded} focused hours per day, and saved the full roadmap.`,
      `Created "${result.goal.title}" with ${result.tasks.length} starter tasks.`
    ),
    { goal: result.goal, starterTasks: result.tasks, goalPlan }
  );
}
...
async function executeTwinSimulation(state, style, sourceMessage) {
  await recordChatPattern(sourceMessage, "plan");
  const result = simulateTwinResponse(state, { prompt: sourceMessage }, buildRuntimeContext(state));
  await addArtifact({
    kind: "digital-twin-simulation",
    title: "Chat digital twin simulation",
    payload: result,
    category: "analysis"
  });
  ...
  return replyAndStore(
    ...
    formatReply(
      style,
      `I ran your digital twin. ${result.likelyAction} Buksy currently puts the odds at ${result.probability}%.`,
      `Twin says ${result.probability}% likely. ${result.likelyAction}`
    ),
    { twinSimulation: result }
  );
}

async function executeAutopilotPreview(state, style, sourceMessage) {
  await recordChatPattern(sourceMessage, "plan");
  const result = runLifeAutopilot(state, buildRuntimeContext(state));
  const focusLine = (result.focusNow || []).map((item) => item.title).join(", ") || "No clear focus yet";
  await addArtifact({
    kind: "autopilot-run",
    title: "Chat autopilot preview",
    payload: result,
    category: "automation"
  });
  ...
  return replyAndStore(
    ...
    formatReply(
      style,
      `${result.message} Focus now: ${focusLine}.`,
      `Autopilot focus: ${focusLine}.`
    ),
    { autopilot: result }
  );
}

async function executeFutureTimelineOverview(state, style, sourceMessage) {
  await recordChatPattern(sourceMessage, "plan");
  const future = buildFutureTimelines(state, buildRuntimeContext(state));
  const balanced = (future.modes || []).find((mode) => mode.id === "balanced");
  await addArtifact({
    kind: "future-timeline",
    title: "Chat future timeline",
    payload: future,
    category: "planning"
  });
  ...
  return replyAndStore(
    ...
    formatReply(
      style,
      balanced
        ? `${future.summary} Balanced mode clears your current open work in about ${balanced.projectedDaysToClearOpenWork} days.`
        : future.summary,
      balanced
        ? `Balanced mode: about ${balanced.projectedDaysToClearOpenWork} days to clear open work.`
        : future.summary
    ),
    { futureTimelines: future }
  );
}

async function executeStrategyOverview(state, style, sourceMessage) {
  await recordChatPattern(sourceMessage, "plan");
  const strategy = buildStrategyEngine(state, buildRuntimeContext(state));
  const future = buildFutureTimelines(state, buildRuntimeContext(state));
  const lead = strategy.focusAreas?.[0];
  await addArtifact({
    kind: "strategy",
    title: "Chat strategy overview",
  ...
  return replyAndStore(
    ...
    formatReply(
      style,
      lead
        ? `${strategy.recommendation} Buksy's recommended future mode is ${future.recommendedMode || "Balanced"}.`
        : "Create a project or goal first so Buksy can rank ROI and future paths.",
      lead
        ? `${strategy.recommendation} Mode: ${future.recommendedMode || "Balanced"}.`
        : "Create a project or goal first."
    ),
    { strategy, futureTimelines: future }
  );
}
...
async function handleChat(rawMessage) {
  ...
  if (looksLikeExecutionRequest(message)) {
    return executeAutonomousRequest(state, style, message);
  }
  ...
  if (looksLikeTwinSimulation(message)) {
    state = await readState();
    style = replyStyleForState(state);
    return executeTwinSimulation(state, style, message);
  }

  if (looksLikeAutopilotPreview(message)) {
    state = await readState();
    style = replyStyleForState(state);
    return executeAutopilotPreview(state, style, message);
  }

  if (looksLikeFutureTimeline(message)) {
    state = await readState();
    style = replyStyleForState(state);
    return executeFutureTimelineOverview(state, style, message);
  }

  if (looksLikeStrategyRequest(message)) {
    state = await readState();
    style = replyStyleForState(state);
    return executeStrategyOverview(state, style, message);
  }
